...
/**
 * Tests if data can be uploaded to Irys using BaseEth.
 * 
 * @returns {Promise<boolean>} Returns true if upload is successful, otherwise false.
 */
export const canUploadBaseEthereum = async (): Promise<boolean> => {
  try {
    // Initialize the Irys Uploader with the Ethereum wallet
    const irysUploader = await Uploader(BaseEth).withWallet(process.env.EVM_PRIVATE_KEY);

...
/**
 * Tests if data can be uploaded to Irys using Bera.
 * 
 * @returns {Promise<boolean>} Returns true if upload is successful, otherwise false.
 */
export const canUploadBera = async (): Promise<boolean> => {
  try {
    // Initialize the Irys Uploader with the Ethereum wallet
    const irysUploader = await Uploader(Bera).withWallet(process.env.EVM_PRIVATE_KEY);

...
/**
 * Tests if data can be uploaded to Irys using Arbitrum.
 * 
 * @returns {Promise<boolean>} Returns true if upload is successful, otherwise false.
 */
export const canUploadArbitrum = async (): Promise<boolean> => {
  try {
    // Initialize the Irys Uploader with the Ethereum wallet
    const irysUploader = await Uploader(Arbitrum).withWallet(process.env.EVM_PRIVATE_KEY);

...
/**
 * Tests if data can be uploaded to Irys using Matic.
 * 
 * @returns {Promise<boolean>} Returns true if upload is successful, otherwise false.
 */
export const canUploadPolygon = async (): Promise<boolean> => {
  try {
    // Initialize the Irys Uploader with the Ethereum wallet
    const irysUploader = await Uploader(Matic).withWallet(process.env.EVM_PRIVATE_KEY);

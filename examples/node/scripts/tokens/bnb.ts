...
/**
 * Tests if data can be uploaded to Irys using BNB.
 * 
 * @returns {Promise<boolean>} Returns true if upload is successful, otherwise false.
 */
export const canUploadBnb = async (): Promise<boolean> => {
  try {
    // Initialize the Irys Uploader with the Ethereum wallet
    const irysUploader = await Uploader(BNB).withWallet(process.env.EVM_PRIVATE_KEY);

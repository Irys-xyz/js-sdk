...
let balpad = false,
  walpad = false; // padding state variables

// Define the CLI flags for the program
program
  .option(
    '-h, --host <string>',
    'Irys node hostname/URL (eg http://node1.irys.xyz)'
  )
  .option('-n, --network <string>', 'The network to use', 'mainnet')
  .option(
    '-w, --wallet <string>',
    'Path to keyfile or the private key itself',
    'default'
  )
  .option('-t, --token <string>', 'The token to use')
  .option(
    '-c --currency <string>',
    'DEPRECATED: the currency to use (same as token)'
  )
  .option(
    '--timeout <number>',
    'The timeout (in ms) for API HTTP requests - increase if you get timeouts for upload'
  )
  ...
  .option(
    '-d, --debug',
    'Increases verbosity of errors and logs additional debug information. Used for troubleshooting.',
    false
  )
  .option(
    '--index-file <string>',
    'Name of the file to use as an index for upload-dir manifests (relative to the path provided to upload-dir).'
  )
  .option('--provider-url <string>', 'Override the provider URL')
  .option('--contract-address <string>', 'Override the contract address')
  .option(
    '--content-type <string>',
    'Override the content type for *ALL* files uploaded'
  )
  .option(
    '--remove-deleted',
    'Removes previously uploaded (but now deleted) items from the manifest'
  )
  .option(
    '--force-chunking',
    'Forces usage of chunking for all files regardless of size'
  );
// Define commands
...
program
  ...
  .action(async (address: string) => {
    try {
      ...
      console.log(
        `Balance: ${balance} ${irys.tokenConfig.base[0]} (${irys.utils.unitConverter(balance).toFixed()} ${irys.token})`
      );
    } catch (err: any) {
      console.error(
        `Error whilst getting balance: ${options.debug ? err.stack : err.message} `
      );
      return;
    }
  });
...
program
  ...
  .action(async (amount: string) => {
    try {
      ...
      console.error(
        `Error whilst sending withdrawal request: ${options.debug ? err.stack : err.message} `
      );
      return;
    }
  });
...
program
  ...
  .action(async (file: string) => {
    try {
      ...
      console.error(
        `Error whilst uploading file: ${options.debug ? err.stack : err.message} `
      );
      return;
    }
  });
...
program
  .command('deploy')
  .description(
    "(DEPRECATED - use the functionally identical 'upload-dir' instead.) Deploys a folder (with a manifest) to the specified bundler"
  )
  .argument('<folder>', 'relative path to the folder you want to deploy')
  .action(async (folder: string) => {
    console.warn(
      "WARN: Deploy is deprecated, use the functionally identical 'upload-dir' instead."
    );
    await uploadDir(folder);
  });
...
async function uploadDir(folder: string): Promise<void> {
  try {
    ...
    console.error(
      `Error whilst uploading ${folder} - ${options.debug ? err.stack : err.message}`
    );
  }
}
...
program
  ...
  .action(async (amount: string) => {
    try {
      ...
      console.error(
        `Error whilst funding: ${options.debug ? err.stack : err.message} `
      );
      return;
    }
  });
...
program
  ...
  .action(async (bytes: string) => {
    try {
      ...
      console.error(
        `Error whilst getting price: ${options.debug ? err.stack : err.message} `
      );
      return;
    }
  });
  ...
  if (!['balance', 'price'].includes(operation)) {
    ...
    if (opts.wallet === 'default') {
      ...
      wallet = await loadWallet(
        walpad ? opts.wallet.substring(1) : opts.wallet
      );
    }
  }
  try {
    ...
    ); /* new getToken(opts.token.toLowerCase())() */
    wallet = wallet ?? '';
    ...
    throw new Error(
      `Error initialising Irys client - ${options.debug ? err.stack : err.message}`
    );
  }
...
if (isScript) {
  ...
  const wal =
    (!argv.includes('-w') ? argv.indexOf('--wallet') : argv.indexOf('-w')) + 1;

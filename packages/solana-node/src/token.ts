...
export type SolanaTokenConfig = TokenConfig<
  any,
...
export default class SolanaConfig extends BaseNodeToken {
  ...
  declare config: SolanaTokenConfig;
  ...
  async getTx(txId: string): Promise<Tx> {
    ...
    const staticAccountKeys =
      stx.transaction.message.getAccountKeys().staticAccountKeys;
    const tx: Tx = {
  ...
  async getCurrentHeight(): Promise<BigNumber> {
    return new BigNumber(
      (await (await this.getProvider()).getEpochInfo()).blockHeight ?? 0
    );
  }
  ...
  async getFee(
    ...
    const connection = await this.getProvider();
    const unsignedTx = await this._createTxUnsigned(
      amount,
      to ?? 'DHyDV2ZjN3rB6qNGXS48dP5onfbZd3fAEz6C5HJwSqRD'
    );
    const computeBudget = new BigNumber(
      (await unsignedTx.getEstimatedFee(connection)) ?? 5000
    );
    ...
    const prioAvg = (recentPrio as { prioritizationFee: number }[])
      .reduce(
        (n: BigNumber, p) => n.plus(p.prioritizationFee),
        new BigNumber(0)
      )
    ...
    const blockHashInfo = await retry(
      async (bail) => {
        try {
          ...
          ).getLatestBlockhash(this.finality);
        } catch (e: any) {

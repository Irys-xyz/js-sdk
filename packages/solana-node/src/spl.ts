...
export class SPLConfig extends SolanaConfig {
  ...
  async _createTxUnsigned(amount: BigNumber.Value, to: string, fee?: GetFeeResult): Promise<Transaction> {
    ...
    const fromTokenAccount = await getOrCreateAssociatedTokenAccount(connection, keys, mint, keys.publicKey);
    const toTokenAccount = await getAssociatedTokenAddress(mint, new PublicKey(to));


    transaction.add(
      createTransferInstruction(fromTokenAccount.address, toTokenAccount, keys.publicKey, +new BigNumber(amount).toNumber())
    );

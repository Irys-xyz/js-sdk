...
export function augmentViemV2(tokenConfig: EthereumConfig, opts: any): void {
  ...
  tokenConfig.ready = async function (this: EthereumConfig) {
    await this.getSigner().ready();
    this._address = await walletClient.getAddresses().then((r: any) => r[accountIndex].toString().toLowerCase());
    this.providerInstance = this.wallet;
  }.bind(tokenConfig);

  tokenConfig.getFee = async (_amount) => new BigNumber(0);

  tokenConfig.getSigner = function (this: EthereumConfig) {
    if (!this.signer) {
      this.signer = new InjectedTypedEthereumSigner({
        getSigner: () => ({
          getAddress: async () => walletClient.getAddresses().then((r: any) => r[accountIndex]),
          _signTypedData: async (domain: any, types: any, message: any): Promise<string> => {
  ...
  if ((tokenConfig as ERC20Config).contractAddress) {
    ...
    tokenConfig.createTx = async function (amount: any, to: any) {
      const config = {

...
type ConnectionInfo = Exclude<
  ConstructorParameters<typeof JsonRpcProvider>[0],
...
export default class BaseEthereumToken extends BaseNodeToken {
  ...
  async createTx(
    ...
    const gasEstimate = fee
      ? EthBigNumber.from(
          new BigNumber(fee).dividedToIntegerBy(gasPrice.toString()).toFixed()
        )
      : undefined;
    ...
    const txId =
      '0x' + keccak256(Buffer.from(signedTx.slice(2), 'hex')).toString('hex');
